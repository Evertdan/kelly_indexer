...
def test_load_from_environment_variables(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert settings_instance.distance_metric == distance_val.capitalize() # Validador capitaliza
        ...
        # assert settings_instance.move_processed_files is True # <-- LÍNEA ELIMINADA (Opción B)
...
def test_distance_metric_validation(monkeypatch):
    """Verifica la validación de la métrica de distancia."""
    monkeypatch.setenv("QDRANT_URL", "http://localhost:6333")

    valid_inputs = ["Cosine", "cosine", "DOT", "dot", "Euclid", "euclid", "Euclidean"]
    expected_outputs = ["Cosine", "Cosine", "Dot", "Dot", "Euclid", "Euclid", "Euclid"]
    ...
    monkeypatch.setenv("DISTANCE_METRIC", "Manhattan")
    with pytest.raises(ValidationError) as excinfo:
        Settings(_env_file=None)
...
def test_load_from_dotenv_file(tmp_path):
    ...
    env_content = """
    QDRANT_URL = "http://dotenv-qdrant:1234"
    QDRANT_COLLECTION_NAME = "dotenv-collection"
    ...
    assert settings_instance.distance_metric == "Cosine"
    assert settings_instance.embedding_model_name == "all-MiniLM-L6-v2"